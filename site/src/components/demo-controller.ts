...
// bid<->total) is the extension's own, bridged in via ../lib/extension-calc.ts.
import {
  fmtMoney,
  pctText,
  calcTotal,
  ...
  bidFromTotalParts,
  bidPerUnitText,
} from "../lib/extension-calc.ts";

type Teardown = () => void;

export function initDemo(box: HTMLElement | null, calc: HTMLElement | null): Teardown {
  if (!box || !calc) return () => {};
  ...
  const amountEl = box.querySelector<HTMLElement>(".ebay-estimation__amount")!;
  const subEl = box.querySelector<HTMLElement>(".ebay-estimation__sub")!;
  const panel = box.querySelector<HTMLElement>(".ebay-estimation__panel")!;
  const toggle = box.querySelector<HTMLElement>(".ebay-estimation__toggle")!;
  const countEl = box.querySelector<HTMLInputElement>(".ebay-estimation__count")!;
  const perEl = box.querySelector<HTMLElement>(".ebay-estimation__perunit")!;
  const shipBtn = box.querySelector<HTMLElement>(".ebay-estimation__ship")!;
  ...
  on(countEl, "change", function () {
    commitCount(parseCount(countEl.value));
  });
  on(countEl, "blur", function () {
    commitCount(parseCount(countEl.value));
  });
  ...
  const bidField = calc.querySelector<HTMLElement>(".ebay-bid-calc__field--bid")!;
  const totalFieldW = calc.querySelector<HTMLElement>(".ebay-bid-calc__field--total")!;
  const bidIn = calc.querySelector<HTMLInputElement>(".ebay-bid-calc__input")!;
  const totIn = calc.querySelector<HTMLInputElement>(".ebay-bid-calc__total")!;
  const calcEl = calc.querySelector<HTMLElement>(".ebay-bid-calc__calc")!;
  const totalCalcEl = calc.querySelector<HTMLElement>(".ebay-bid-calc__total-calc")!;
  const bidPerEl = calc.querySelector<HTMLElement>(".ebay-bid-calc__perunit")!;
  const bidSubEl = calc.querySelector<HTMLElement>(".ebay-bid-calc__sub")!;
  let lastTotal: number | null = null,
    lastTotalNoShip: number | null = null,
    lastValid = false;
  ...
  function renderBidPerUnit() {
    if (lastValid && perUnitActive) {
      bidPerEl.textContent =
        "(" + bidPerUnitText(lastTotal!, lastTotalNoShip, includeShip, count) + ")";
