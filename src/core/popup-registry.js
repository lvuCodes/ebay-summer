...
//
// registerPopupSection({ id, title, master, column, render, controls, toggles, init })
//   id        section id; the chrome becomes #header-<id> / #body-<id>
//   title     the group header text
//   master    optional storage key for the section's header on/off switch
//   column    1 or 2 — which popup column the section renders into
//   render(body)  fills the section body with the feature's controls
//   controls  { fields, checks, selects, texts, radios, collapses } — any subset;
//             popup.js merges these into the tables its load/save loop walks
//   toggles   [{ control, sections }] reveal/hide rules, applied on change
//   init(ctx) optional wiring the generic loop cannot express (a preview button,
//             a clear button, a dependent row). ctx = { save, load }
//
