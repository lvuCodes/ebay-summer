...
(function () {
  ...
  // A listing where the seller sent you an offer has two Best-Offer lightboxes, both
  // fetched into `.app-siolayer-main-container` (the seller-initiated-offer layer) and
  // both ending in an `.app-sio-ctas` action group. We seat our widgets just above
  // that group, mirroring the on-listing placement (calc above the action button):
  //  - "Review offer" modal: an offer-details table (Seller offer / Shipping /
  //    Estimated total) + Go-to-checkout / Make-counteroffer / Decline. We add the
  //    purple est-total box on the OFFER price (with the user's sales tax folded in,
  //    which eBay's own modal total omits) AND the orange calculator.
  //  - "Make counteroffer" modal: a price input where you type YOUR counter (in the
  //    seller's currency, "doesn't include shipping") and an empty details table. We
  //    add ONLY the orange calculator — a USD landed-total helper for the counter —
  //    since there's no seller-offer total to box here.
  // The modals overlay the listing page, so this runs inside renderListing.
